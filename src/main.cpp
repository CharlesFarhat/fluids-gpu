...
bool vsync = true; // redraw synced with screen refresh
...
// Some problems with the dawing and the parametric render
//--------------------------------------------------------------------------------------
//***********************************************************************************
...
//***********************************************************************************
{
    ...
    wcex.lpszClassName = reinterpret_cast<LPCSTR>(L"Wave3DWindowClass");
    ...
    g_hWnd = CreateWindow(reinterpret_cast<LPCSTR>(L"Wave3DWindowClass"), reinterpret_cast<LPCSTR>(L""), WS_OVERLAPPEDWINDOW, rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top, NULL, NULL, hInstance, NULL );
...
*/
    ...
    else switch( msg)
        {
            ...
            {
                DestroyWindow( hWnd );
                UnregisterClass(reinterpret_cast<LPCSTR>(L"Wave3DWindowClass"), NULL );
                return 0;
            }
...
//*************************************************************************
void SetConstantBuffer(bool activate)
//*************************************************************************
{
...
// Fill constant buffer
    ...
    {
        mc.mouse[0] = (float)450; //  position x
        mc.mouse[1] = (float)750; // position y
        mc.mouse[2] = viewSlice;
        mc.mouse[3] = 4; // radius in voxels;

        // input vector position

        mc.dragDirection[0] = (float)1*DimX / g_width;
        mc.dragDirection[1] = (float)1*DimY / g_height;
...
//*************************************************************************
{
    ...
    {  float td = t - timer; // real time interval a bit more than a second
        ...
        if (smoky)
            swprintf(message,L"Fluid3D Charles200000 Smoke     FPS %.2f", (float)fps/td);
        else
            swprintf(message,L"Fluid3D Charles200000 Fire     FPS %.2f", (float)fps/td);
        SetWindowText(g_hWnd, reinterpret_cast<LPCSTR>(message));
        fps=0;
    }
    fps++;
